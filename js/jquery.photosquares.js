...
(function($) {
	...
	*/
	$.fn.jsquares = $.fn.jSquares = function(endAnimationCallback, options){
		// General
		var main = this,
			image_counter = 0,
			dropin_int = 0,
			...
			defaults = {
				...
				cover: 'blackCover', // target (div) holding info
				...
				fade_to: 0.3, // fade image/caption to what..
				fade_start: 1, // send image back to...
				fade_speed: 50, // int or string: anytime the fade effect is used, how fast should it be
				shuffle_in: true, // have the pictures all fade in on page load?
				shuffle_in_speed: 90, // how long to wait before we fade in the next image on page load
		...
		var mycallback = endAnimationCallback;
		// Adjust the var is people resize!
		$(window).resize(function(){
			window_width = $(window).width();
		});
			
		if (options.hover_animation)
		{
			main.find(options.image).children("a").append($("<div class='blackCover'>"));
		}
		...
		*/
		...
		/**
		* Add all listeners for the animation with mouseover
		* @private
		* @param {Array} images An array of all images in the stack
		*/
		function _addFadeListeners(images) {
			if (!options.hover_animation || main.length === 0)
				return;
			$(images).each(function (i) {
				var img = $(options.image + ':eq(' + images[i] + ')', main);
				img.mouseover(function() {
					enteredImage = true;
					main.find("div."+options.cover).css("display", "block");
					$(this).children().children("img").css({zIndex:3, position:"absolute"});
				});
				img.mouseout(function () {
					$(this).children().children("img").css({zIndex:0});
				});
			});
			$(main).mouseout(function () {
				...
				setTimeout(function () {
					if (!enteredImage) {
						main.find("div."+options.cover).css("display", "none");
					}
				}, 50);
			});
		}
		...
		*/
		...
		return this.each(function() {
			...
			if (options.shuffle_in)
				_setImageTimeoutSlideDown(images);
			else {
				$image.show();
				_addFadeListeners(images);
			}
